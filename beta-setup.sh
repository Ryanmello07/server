...
ensure_secrets() {
    if [ ! -f "$SECRETS_FILE" ]; then
        ...
        PROXY_SECRET=$(rand32)
        PROVIDER_EGRESS_INGEST_SECRET=$(rand32)
...
PROVIDER_EGRESS_INGEST_SECRET=$PROVIDER_EGRESS_INGEST_SECRET
EOF
...
EOF
    ...
    # Shared secret the operator-run provider-egress-geolocation prober
    # presents when submitting a probed egress location. The ingest endpoint
    # is disabled (fails closed) until this file exists with a non-empty
    # ingest_secret.
    cat > "$VAULT/provider_egress.yml" <<EOF
# Generated by beta-setup.sh
ingest_secret: "${PROVIDER_EGRESS_INGEST_SECRET}"
EOF
}
